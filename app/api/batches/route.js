...
export async function POST(request) {
  try {
    ...
    // Collect all user IDs to enrol
    const userIdsToEnrol = [...existingStudentIds];
    ...
    for (const s of newStudents) {
      ...
      userIdsToEnrol.push(user.id);
    }

    // Enrol all students in the new batch via the join table
    ...
    if (userIdsToEnrol.length) {
      await prisma.batchStudent.createMany({
        data: userIdsToEnrol.map((userId) => ({ batchId: batch.id, userId })),
        skipDuplicates: true,
      });
    }
